 ...
 *
 * Doc comments: copyright [MDN
 * Contributors](https://developer.mozilla.org/en-US/docs/MDN/Community/Roles_teams#contributor)
...
exports.status = {
     ...
     * response](/en-US/docs/Web/HTTP/Status#information_responses) may be sent by a server while
     ...
     * [preconnect](/en-US/docs/Web/HTML/Attributes/rel/preconnect) to sites or start
     * [preloading](/en-US/docs/Web/HTML/Attributes/rel/preload) resources even before the server
     * has prepared and sent that final response.
     *
     * The early hint response is primarily intended for use with the
     * [`Link`](/en-US/docs/Web/HTTP/Headers/Link) header, which indicates the resources to be
     * loaded. It may also contain a [`Content-Security-Policy`](/en-US/docs/Web/HTTP/CSP) header
     ...
     * recommended](https://www.rfc-editor.org/rfc/rfc8297#section-3) to only send HTTP `103 Early
     * Hints` responses over HTTP/2 or later, unless the client is known to handle informational
     * responses correctly.
     *
     * Most browsers limit support to HTTP/2 or later for this reason. See [browser
     * compatibility](#browser_compatibility) below.
     *
     ...
     * range-14](https://en.wikipedia.org/wiki/HTTPRange-14) — or an upload-progress page). This
     * response code is often sent back as a result of [`PUT`](/en-US/docs/Web/HTTP/Methods/PUT) or
     ...
     * conditional [`GET`](/en-US/docs/Web/HTTP/Methods/GET) or
     * [`HEAD`](/en-US/docs/Web/HTTP/Methods/HEAD) request with an
     * [`If-None-Match`](/en-US/docs/Web/HTTP/Headers/If-None-Match) or an
     * [`If-Modified-Since`](/en-US/docs/Web/HTTP/Headers/If-Modified-Since) header and the
     * condition evaluates to false. It is an implicit redirection to a cached resource that would
     * have resulted in a [`200`](/en-US/docs/Web/HTTP/Status/200) `OK` response if the condition
     * evaluated to true.
     *
     * The response must not contain a body and must include the headers that would have been sent
     * in an equivalent [`200`](/en-US/docs/Web/HTTP/Status/200) `OK` response:
     ...
     * panels](https://firefox-source-docs.mozilla.org/devtools-user/network_monitor/index.html) of
     ...
     * download](https://developers.google.com/drive/api/guides/manage-uploads) on Google Drive
     * documentation.)
     *
     * Documentation: copyright [MDN
     * Contributors](https://developer.mozilla.org/en-US/docs/MDN/Community/Roles_teams#contributor)
     ...
     * technology](/en-US/docs/MDN/Writing_guidelines/Experimental_deprecated_obsolete#experimental)**
     * Check the [Browser compatibility table](#browser_compatibility) carefully before using this
     * in production.
     *
     ...
     * Forbidden`** status code, re-authenticating makes no difference. The access is tied to the
 ...
 *
 * The HTTP **`103 Early Hints`** [information
 * response](/en-US/docs/Web/HTTP/Status#information_responses) may be sent by a server while it is
 * still preparing a response, with hints about the sites and resources that the server is
 * expecting the final response will link. This allows a browser to
 * [preconnect](/en-US/docs/Web/HTML/Attributes/rel/preconnect) to sites or start
 * [preloading](/en-US/docs/Web/HTML/Attributes/rel/preload) resources even before the server has
 * prepared and sent that final response.
 *
 * The early hint response is primarily intended for use with the
 * [`Link`](/en-US/docs/Web/HTTP/Headers/Link) header, which indicates the resources to be loaded.
 * It may also contain a [`Content-Security-Policy`](/en-US/docs/Web/HTTP/CSP) header that is
 * enforced while processing the early hint.
 *
 * A server might send multiple `103` responses, for example, following a redirect. Browsers only
 * process the first early hint response, and this response must be discarded if the request
 * results in a cross-origin redirect. Preloaded resources from the early hint are effectively
 * pre-pended to the `Document`'s head element, and then followed by the resources loaded in the
 * final response.
 *
 * **Note:** For compatibility reasons [it is
 * recommended](https://www.rfc-editor.org/rfc/rfc8297#section-3) to only send HTTP `103 Early
 ...
 * and has led to the creation of a resource. The new resource, or a description and link to the
 * new resource, is effectively created before the response is sent back and the newly created
 * items are returned in the body of the message, located at either the URL of the request, or at
 * the URL in the value of the [`Location`](/en-US/docs/Web/HTTP/Headers/Location) header.
 *
 * The common use case of this status code is as the result of a
 * [`POST`](/en-US/docs/Web/HTTP/Methods/POST) request.
 *
 ...
 * more than one possible response. The user-agent or the user should choose one of them. As there
 ...
 * (such as a confirmation page, a representation of a real-world object — see [HTTP
 * range-14](https://en.wikipedia.org/wiki/HTTPRange-14) — or an upload-progress page). This
 * response code is often sent back as a result of [`PUT`](/en-US/docs/Web/HTTP/Methods/PUT) or
 ...
 * to retransmit the requested resources. This response code is sent when the request is a
 * conditional [`GET`](/en-US/docs/Web/HTTP/Methods/GET) or
 * [`HEAD`](/en-US/docs/Web/HTTP/Methods/HEAD) request with an
 * [`If-None-Match`](/en-US/docs/Web/HTTP/Headers/If-None-Match) or an
 * [`If-Modified-Since`](/en-US/docs/Web/HTTP/Headers/If-Modified-Since) header and the condition
 * evaluates to false. It is an implicit redirection to a cached resource that would have resulted
 * in a [`200`](/en-US/docs/Web/HTTP/Status/200) `OK` response if the condition evaluated to true.
 *
 * The response must not contain a body and must include the headers that would have been sent in
 * an equivalent [`200`](/en-US/docs/Web/HTTP/Status/200) `OK` response:
 * [`Cache-Control`](/en-US/docs/Web/HTTP/Headers/Cache-Control),
 * [`Content-Location`](/en-US/docs/Web/HTTP/Headers/Content-Location),
 * [`Date`](/en-US/docs/Web/HTTP/Headers/Date), [`ETag`](/en-US/docs/Web/HTTP/Headers/ETag),
 * [`Expires`](/en-US/docs/Web/HTTP/Headers/Expires), and
 ...
 * **Experimental:** **This is an [experimental
 * technology](/en-US/docs/MDN/Writing_guidelines/Experimental_deprecated_obsolete#experimental)**
 * Check the [Browser compatibility table](#browser_compatibility) carefully before using this in
 * production.
 *
 ...
 * ## 409 Conflict
 *
 * The HTTP **`409 Conflict`** response status code indicates a request conflict with the current
 * state of the target resource.
 *
 * Conflicts are most likely to occur in response to a [`PUT`](/en-US/docs/Web/HTTP/Methods/PUT)
 * request. For example, you may get a 409 response when uploading a file that is older than the
 ...
 * ## 413 Content Too Large
 *
 * The HTTP **`413 Content Too Large`** response status code indicates that the request entity is
 * larger than limits defined by server; the server might close the connection or return a
 * [`Retry-After`](/en-US/docs/Web/HTTP/Headers/Retry-After) header field.
 *
 * Prior to RFC 9110 the response phrase for the status was **`Payload Too Large`**. That name is
 * still widely used.
 *
 * Documentation: copyright [MDN
 * Contributors](https://developer.mozilla.org/en-US/docs/MDN/Community/Roles_teams#contributor)
 ...
 * @see {@link https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/422}
 */
exports.unprocessableContent = 422;
/**
 * ## Locked
 *
 * **[WebDAV](https://developer.mozilla.org/en-US/docs/Glossary/WebDAV)**
